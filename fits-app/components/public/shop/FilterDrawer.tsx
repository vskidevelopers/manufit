// components/public/shop/FilterDrawer.tsx
'use client';
...
export function FilterDrawer({ children }: FilterDrawerProps) {
    ...
    return (
        ...
        `}
            ...
            <DrawertoggleHandler onToggle={() => setIsOpen((prev) => !prev)} />
        </>
    );
}

function DrawertoggleHandler({ onToggle }: { onToggle: () => void }) {
    useEffect(() => {
        const toggleBtn = document.querySelector('[data-shop-filter-toggle]');
        if (!toggleBtn) return;
